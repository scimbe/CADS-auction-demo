#!/usr/bin/env bash
# Enable/disable the auction-demo.bunsenbrenner.org demo — same publishing shape as
# CADS-Tunnel's examples/help-site/run-demo.sh and CADS-flappy-demo/run-demo.sh.
#
#   ./run-demo.sh up      # enable  (default) — mint token, deploy, wait for HTTPS
#   ./run-demo.sh down    # disable — take the demo offline (stop bridge+origin+agent)
#   ./run-demo.sh status  # show container status
#
# `up` assumes a RUNNING CADS-Tunnel plane reachable from this host: ct-edge (with
# CT_EDGE_BROWSER_LISTEN=:443) and ct-control-plane. It mints a join token, brings up
# the bridge + Caddy origin (real LE cert via deSEC DNS-01, issued CORE-side and
# relayed in) + a Browser-Plane agent bound to the hostname, then polls until the
# page is served over HTTPS. Prereqs: see CADS-Tunnel's docs/dns01-desec.md.
#
# Unlike flappy-demo/cookbook-demo, this repo is standalone (not nested inside a
# CADS-Tunnel checkout) — set ENV_FILE to wherever the plane's admin-token env
# actually lives (e.g. /path/to/CADS-Tunnel/docker/deploy/.env) if you need it.
set -euo pipefail
cd "$(dirname "$0")"

CMD="${1:-up}"
COMPOSE="docker compose -f compose.auction-demo.yml"
ENV_FILE="${ENV_FILE:-.env}"
[ -f "$ENV_FILE" ] && set -a && . "$ENV_FILE" && set +a || true

HOSTNAME_FQDN="${HOSTNAME_FQDN:-auction-demo.bunsenbrenner.org}"
...
if [ "$CMD" = "down" ] || [ "$CMD" = "disable" ] || [ "$CMD" = "off" ]; then
  say "Taking the auction-demo offline (stopping bridge + origin + agent)"
  $COMPOSE down
  printf '\033[32m✓ auction-demo is OFFLINE.\033[0m\n'
  exit 0
fi
if [ "$CMD" = "status" ]; then
  $COMPOSE ps
...
AUCTION_CERT_DIR="${AUCTION_CERT_DIR:?set AUCTION_CERT_DIR=<dir with fullchain.pem+privkey.pem from the operator>}"
[ -f "$AUCTION_CERT_DIR/fullchain.pem" ] && [ -f "$AUCTION_CERT_DIR/privkey.pem" ] \
  || die "no fullchain.pem/privkey.pem in AUCTION_CERT_DIR=$AUCTION_CERT_DIR — ask the operator to authorize $HOSTNAME_FQDN and relay the cert files"
...
# #214) can skip minting entirely by pre-setting AUCTION_JOIN_TOKEN -- consumes
# one of the batch instead of calling the admin-gated /enroll/issue.
if [ -n "${AUCTION_JOIN_TOKEN:-}" ]; then
  say "Using pre-minted AUCTION_JOIN_TOKEN (skipping /enroll/issue -- no admin token needed)"
  TOKEN="$AUCTION_JOIN_TOKEN"
else
  ...
  [ -n "$TOKEN" ] || die "could not mint a join token (if the CP gates /enroll/issue, set CT_CP_EDGE_ADMIN_TOKEN in $ENV_FILE, or set AUCTION_JOIN_TOKEN to a pre-minted one)"
...
if [ -n "$EDGE_ADMIN_URL" ] && [ -n "$EDGE_ADMIN_TOKEN" ]; then
  ...
  AUCTION_AGENT_TOKEN="$(openssl rand -hex 32)"
  say "Authorizing $HOSTNAME_FQDN at the edge (hostname-ownership, BP4b)"
  curl -fsS -X POST "${EDGE_ADMIN_URL%/}/admin/authorize-host/$AUCTION_AGENT_TOKEN/$HOSTNAME_FQDN" \
...
for i in $(seq 1 60); do
  if curl -fsS --max-time 5 "https://$HOSTNAME_FQDN/" >/dev/null 2>&1; then
    printf '\033[32m✓ LIVE — https://%s/ serves the live auction dashboard.\033[0m\n' "$HOSTNAME_FQDN"
    exit 0
  fi
  sleep 5
done
...
echo "     - cert:    $COMPOSE logs auction-demo-origin   (deSEC DNS-01 progress)"
echo "     - agent:   $COMPOSE logs auction-demo-agent     (onboard + hostname bind)"
echo "     - bridge:  $COMPOSE logs auction-demo-bridge    (auction logic)"
